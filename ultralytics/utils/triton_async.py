...
class AsyncTritonRemoteModel:
    ...
    def __init__(self, url: str, endpoint: str = "", scheme: str = ""):
        ...
        if scheme == "http":
            ...
            import tritonclient.http.aio as async_client  # noqa
            ...
            self.triton_client_async = async_client.InferenceServerClient(
                url=self.url, verbose=False, ssl=False
            )
        ...
        self.AsyncInferRequestedOutput = async_client.InferRequestedOutput
        self.AsyncInferInput = async_client.InferInput
    ...
    async def async_call(self, *inputs: np.ndarray) -> List[np.ndarray]:
        """
        Call the model with the given inputs asynchronously.
        ...
        for i, x in enumerate(inputs):
            if x.dtype != self.np_input_formats[i]:
                x = x.astype(self.np_input_formats[i])
            infer_input = self.AsyncInferInput(
                self.input_names[i],
                [*x.shape],
                self.input_formats[i].replace("TYPE_", ""),
            )
