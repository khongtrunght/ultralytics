...
class BasePredictor:
    ...
    async def async_inference(self, im, *args, **kwargs):
        """Runs inference on a given image asynchronously using the specified model and arguments."""
        visualize = (
            increment_path(self.save_dir / Path(self.batch[0][0]).stem, mkdir=True)
            if self.args.visualize and (not self.source_type.tensor)
            else False
        )
        return await self.model.async_call(im, augment=self.args.augment, visualize=visualize, embed=self.args.embed, *args, **kwargs)
    ...
    async def async_call(self, source=None, model=None, stream=False, *args, **kwargs):
